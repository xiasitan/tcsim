...
def D(state: qt.Qobj , t_displace: list,  epsilon, chi=CHI, phase_error = 0,    loss = None) -> qt.Qobj:
    if loss is None:
        ...
        return qt.mesolve(H_dispersiveDrive(chi, epsilon * np.exp(1j * phase_error)), state, t_displace, c_ops = loss).states[-1]
...
def ECD_exp_equal_displacements(state: qt.Qobj, t_displace: list, t_wait: list, alpha_scale,  epsilon, chi = CHI, loss = None):
    if loss == None:
        # Execute ECD sequence as specifiedin in A.Eickbusch paper.
        psi1 = D(state,t_displace, alpha_scale*epsilon, chi) # first displacement
        psi2 = qt.mesolve(H_dispersive(chi), psi1, t_wait).states[-1] # wait for time t_list[-1]
        psi3 = D(psi2,t_displace,  -alpha_scale*epsilon, chi) # first negative displacement
        psi4 = Rx(np.pi)*psi3
...
def V(state: qt.Qobj, t_displace: list, t_wait: list, alpha_scale, epsilon, chi = CHI, phase_error = [0,0,0], loss = None):
    if loss is None:
        ...
        if state.type is 'ket':
            state = qt.ket2dm(state)
        rho1 = Rx(np.pi/2+phase_error[0]) * state * Rx(np.pi/2+phase_error[0]).dag()
        rho2 = ECD_exp(rho1, t_displace, t_wait, -alpha_scale, epsilon, chi, loss)
        rho3 = Rx(-np.pi+phase_error[1]) * rho2 * Rx(-np.pi+phase_error[1]).dag()
        rho4 = Rx(-np.pi/2+phase_error[2]) * rho3 * Rx(-np.pi/2+phase_error[2]).dag()
    return rho4
        


def char_point(state, t_displace: list, t_wait: list, alpha_scale, epsilon, chi = CHI, loss = None, real = True):
    if loss is None:

...
from tcsim.analyze import fit_funcs 
...
for name in os.listdir(os.path.dirname(fit_funcs.__file__)):
    if name == "__init__.py" or not name.endswith(".py"):
        continue
    name = name[:-3]
    mod = importlib.import_module("tcsim.analyze.fit_funcs." + name)
    reload(mod)
...
### new code written for tcsim. Code above is lifted from code base
from tcsim.analyze.fit_funcs.coherent_char_func import func as coherent_char_func 
from tcsim.analyze.fit_funcs.gaussian import func as gaussian_func 

def char_fit_coherent(x,y, initial_guess):
    # This might be possible to do with the fitting code above, however, this is simple to use and easy to manipulate.

    popt, pcov = curve_fit(coherent_char_func, x, y, initial_guess)
    px_fitting = coherent_char_func(x, *popt) # used for plot
    
    fig, ax = plt.subplots()
    ax.plot(x, px_fitting, label = 'fit', )
    ax.plot(x, y, '*', label = 'data') 

    ax.legend()
    ax.set_xlabel(r'$\beta$')
    ax.set_ylabel(r'$Char(\beta)$')

    ax.set_title(f'$a_0$={round(popt[0],3)}, $amp=${round(popt[1],3)}, $offset$ {round(popt[2],3)}')
    print('perr =', np.sqrt(np.diag(pcov)))
    
def char_fit_gaussian(x,y, initial_guess):
    # This might be possible to do with the fitting code above, however, this is simple to use and easy to manipulate.

    popt, pcov = curve_fit(gaussian_func, x, y, initial_guess)
    px_fitting = gaussian_func(x, *popt) # used for plot
    ...
    ax.set_title(f'$x_0$={round(popt[0],5)}, $sigma=${round(popt[1],3)}, $offset=$ {round(popt[2],3)}, amp = {round(popt[3], 6)}')
    print('perr =', np.sqrt(np.diag(pcov)))
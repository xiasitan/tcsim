...
def load_triple_mask_decay(path: str, file_end:str, thresh, plot: bool = True, vac_norm = 1):
...
#         m2 = ss_data[:, 2::3]
        ...
        proj_e = ma.masked_array(m2, mask=np.logical_not(m1)).mean(axis=0).reshape(len(x), len(y)) *1/vac_norm* 2 - 1

        ## condition on the first measurement as well
        double_ps_g = ma.masked_array(m2_g, mask=m1_g).mean(axis=0).reshape(len(x), len(y)) *1/vac_norm* 2 - 1
        double_ps_e = ma.masked_array(m2_g, mask=np.logical_not(m1_g)).mean(axis=0).reshape(len(x), len(y)) *1/vac_norm* 2 - 1

...
def plot_wigner(
    ...
    # Check if state is a tensor product and trace out the qubit if it is.
    # Assuming the state is tensor(qubit,cavity)
    ...
    def plot_qfunc(
        ...
        if contour:
            levels = np.linspace(-1.1, 1.1, 102)
            im = ax.contourf(
                xvec, xvec, Q, cmap="seismic", vmin=-1, vmax=+1, levels=levels,
            )
        else:
            im = ax.pcolormesh(
                xvec, xvec, Q, cmap="seismic", vmin=-1, vmax=+1
            )
...
def plot_char_exp(
    ...
    if real:
        im = ax.pcolormesh(xvec, xvec, (CF), cmap = 'bwr', vmin=-1, vmax=1, shading='auto')
        ax.set_aspect('equal')
        ax.set_xlabel(r"Re$(\alpha)$")
        ax.set_ylabel(r"Im$(\alpha)$")
        ax.set_title(r'$ Re(C(\alpha))$')
...
def plot_2D(xvec, yvec, char_func):
    ...
    F = ax.pcolormesh(xvec, yvec, char_func, cmap='bwr', vmin=-1, vmax=1, shading='auto')
    ax.set_aspect('equal')
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    fig.colorbar(F)
